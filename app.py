...
def main():
    ...
    tb1, tb2 = st.tabs(["Live Feed","Settings"])
    
    with tb1:
        st.title(":green[Drowsiness Detection System]")
        run = st.checkbox('Run')
        ...
        while run:
            ...
            for face in faces:
                ...
                # Draw eye landmarks, yawn count and drowsiness alert (if face detected)
                for point in left_eye_points:
                    cv2.circle(frame, point, 2, (0, 255, 0), -1)
                for point in right_eye_points:
                    cv2.circle(frame, point, 2, (0, 255, 0), -1)
                cv2.putText(frame, f"Yawn Count: {yawn_counter}", (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2) 
